...
5. 懒汉式-方式3（双重检查锁）
       ...
       public static Singleton getInstance() {
           ...
           if(instance == null) {
               synchronized (Singleton.class) {
                   //抢到锁之后再次判断是否为null
                   if(instance == null) {
                    //如果a线程在此时停留在这里，但是由于优化问题instance已经存在
                    //但是b线程会发现instance非空，直接返回，但为赋值所以此时仍然会为空
                       instance = new Singleton();
                   }
               }
           }
   ...
   `volatile`的一个重要特性是 禁止指令重排序，`volatile` 的另一个重要特性是 内存可见性。在多线程环境中，线程可能会将变量缓存在本地内存（线程私有）中，而不是直接从主内存读取。这可能导致一个线程修改了变量，而其他线程看不到最新的值。
   **防止指令重排序**：确保对象的创建过程是原子的，避免线程拿到未完全初始化的对象。
    **保证内存可见性**：确保一个线程创建的实例能够被其他线程正确地看到。
...
6. 懒汉式-方式4（静态内部类方式）
   ...
   ​	**静态内部类单例模式**是一种优秀的单例模式，是开源项目中比较常用的一种单例模式。在没有加任何锁的情况下，保证了多线程下的安全，并且没有任何性能影响和空间的浪费。
...
序列化（Serialization）和反序列化（Deserialization）是计算机科学中两个重要的概念，主要用于将对象的状态转换为可存储或可传输的格式，以及将这种格式还原为对象的过程。
* 序列化反序列化
  ...
  **Test类：**
  ...
  也就是说，对象被序列化输出之后，再反序列化以后得到的就不是同一个对象了，就破坏了单例模式。
...
* 序列化、反序列方式破坏单例模式的解决方法
  ...
  **源码解析：**
  ...
  ```
就是说，定义了readResolve方法，ObjectInputStream类就会自动调用这个方法，而不是自己创建对象
